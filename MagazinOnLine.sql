...
select min(Pret) as celMaiMicPret
from Produse;

-- Show the highest price
select max(Pret) as celMaiMarePret
from Produse;

-- Show the number of products with the same price
select count(*) 
from produse
where pret = 105;
...
-- Show the sum of the prices
select sum(Pret) as sumaPreturilor
from Produse;
...
-- 
create table codIntern
(
ID int not null auto_increment,
codIntern int not null,
primary key (ID)
);

insert into codIntern (codIntern) values
(101),
(102),
(103),
(104),
...
select * from codIntern;

-- Add a column
alter table codIntern
add codProducator varchar (10) not null;

-- Delete a column
alter table codIntern
drop column codProducator;

-- Rename tha table
rename table codIntern to idIntern;

-- Rename column
alter table idIntern
change codIntern idIntern int not null;

-- Truncate table
truncate table idIntern;

select * from idIntern;
